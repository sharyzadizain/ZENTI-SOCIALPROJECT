...
const displayName = computed(() => auth.onboardingName.value.trim() || "Имя");
const confirmationPromptText = computed(() => `Твое имя «${displayName.value}», верно? Если да - скажи «Да», или нажми на кнопку и удерживай 5 секунд если нет подожди 15 секунд!`);
...
function startEditHold(): void {
	...
	holdTimer = setTimeout(() => {
		...
		statusMessage.value = "Открываю экран изменения имени.";
		void editName();
	}, HOLD_TO_EDIT_MS);
...
function mapSpeechError(errorCode: string): string {
	...
	if (errorCode === "audio-capture") {
		return "Микрофон недоступен. Нажми кнопку «Имя верное» для подтверждения.";
	}

	if (errorCode === "not-allowed") {
		return "Нет доступа к микрофону. Разреши доступ или нажми кнопку «Имя верное».";
	}

	if (errorCode === "network") {
		return "Ошибка сети при распознавании. Скажи «Да» еще раз или нажми кнопку «Имя верное».";
	}
...
function startVoiceConfirmation(): void {
	...
	if (!speechRecognition) {
		keepListeningForConfirmation = false;
		statusMessage.value = "В этом браузере нет распознавания речи. Нажми кнопку «Имя верное» для подтверждения.";
		return;
	}
